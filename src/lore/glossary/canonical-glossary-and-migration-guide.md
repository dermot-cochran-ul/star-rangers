...
### Plainmark (settled 2026-08-11)
- **Layer:** Standard (In-Universe, Material)
- **Definition:** The [Orbital Habitats Compact's](/star-rangers/lore/orbital-habitats-compact/) non-alphabetic marking standard — formally **Compact Common Marking**, which nobody says. Stencilled panels of one to four stacked colour bands, each pairing one ground colour with one figure class from a closed set (ochre lozenges for hazard, bone-white chevrons for the way, black strokes and blocks for count, black rings for refuge; a diagonal bar voids any band), placed on the fixed frame beside a hatch or at a corridor junction, never on a moving leaf. Reader-facing entries: [The Plainmark](/star-rangers/lore/plainmark/), [Plainmark (glossary)](/star-rangers/glossary/plainmark/).
- **Notes:** No letters, no numerals, no characters of any script — nothing in it to spell and nothing to mistranslate, which is also why Compact-habitat signage can appear in generated art on purpose rather than being banned outright (see `story-bible/images.md`). It cannot name, instruct, forbid, promise, or speak of anything but *now*, and its figure set is closed: a new sign is a Compact schedule revision, not a per-scene coinage.
- **Scope rule:** A Compact standard, not a universal one — reserve it for Compact-habitat settings. Runs wall to wall through the five member habitats and into dockside bands where mixed hulls berth in numbers, thinning out exactly as fast as that problem does. The Celtic Union letters its own corridors in its members' languages and is explicitly not an adopter. Ancestry runs through generation-ark hull-stencilling and Belt depot custom; the Compact codified an existing habit rather than inventing one.
