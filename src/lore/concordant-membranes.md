...
**The structure nests, and getting the scale right is the commonest correction Survey Corps makes to its own older filings.** A membrane is **universe-scale** — the bounded region one primary universe occupies, and never anything smaller. A [Concordant Zone](/star-rangers/glossary/concordant/) is a region *inside* one — a universe holds very few, and none of them is a membrane in its own right. Few, not many: a zone spans the visible universe, so a universe has room for only a handful, which is the reason internal edges between them are rare and unreachable rather than commonplace. A single [Worldwright](/star-rangers/glossary/worldwright/) authors many primary membranes, since the unit of creation is the universe set rather than the single world; the zones inside each are the unit of consistency, not of authorship.
