...
**The structure nests, and getting the scale right is the commonest correction Survey Corps makes to its own older filings.** A membrane is **universe-scale** — the bounded region one primary universe occupies, and never anything smaller. A [Concordant Zone](/star-rangers/glossary/concordant/) is a region *inside* one — sparse wherever anyone has looked, and none of them a membrane in its own right. **How many a membrane holds is not a settled figure and cannot be**, since a single zone already exceeds everything an observer can see; a handful of unbounded zones and endlessly many merely vast ones are indistinguishable from inside either. What the scale does fix is the operational fact: internal edges between zones lie at least a horizon away from anywhere instruments have reached. A single [Worldwright](/star-rangers/glossary/worldwright/) authors many primary membranes, since the unit of creation is the universe set rather than the single world; the zones inside each are the unit of consistency, not of authorship.
