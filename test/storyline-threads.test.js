...
// deploy-time filter, and the registry's shape carries the privacy
// machinery's assumptions (a season claimed by two threads, or a private
// thread without a homeDomain, would misroute content or excluded-page
// placeholders with no build failure).
...
test("every private thread names a homeDomain for its excluded-page placeholder", () => {
  for (const thread of STORYLINE_THREADS) {
    if (!thread.private) continue;
    assert.ok(thread.homeDomain, `${thread.id} is private but has no homeDomain`);
    // The point of homeDomain is to escape the self-referential loop the
    // default reference domain would create - so it must differ from it.
