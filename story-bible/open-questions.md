...
- **[Quern](../src/lore/planets/quern.md)**, drafted 21 August 2026 from Dermot's tidal-locked-world pitch and not merged — a lore entry asserts about the world, so it is draft-and-stop. It creates a new star system (**Kiln**) and three named surface features (the Glazing, the Millrace, the Meal), and deliberately leaves the balance-or-collapse question unadjudicated in-world on the two-readings pattern — that absence is marked, not an oversight. Nothing else references the world yet, so dropping or rewriting the draft orphans nothing. It has no image; if merged, it joins the lore entries awaiting cards (see `images.md`).
