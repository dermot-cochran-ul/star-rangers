...
const { TIER_ORDER } = require("./editions");
...
// below the build's own. An unknown tier on either side is treated as the
// general tier: the build side because that is where the canonical site sits,
// the block side because validate-content.js refuses an unknown tier in
// `povs:` and a typo here should fail loudly there rather than hide prose.
function povTierVisible(blockTier, buildTier) {
  if (!blockTier) return true;
  const block = tierRank(blockTier);
  const build = tierRank(buildTier);
  const generalRank = TIER_ORDER.indexOf("general");
  return (block === null ? generalRank : block) <= (build === null ? generalRank : build);
}
